    ...
    const build = extend(MessageBlock.MessageBuild, effectTester, {
        ...
        buildConfiguration(table){
            ...
            table.button(Icon.star, Styles.cleari, () => {
                try{
                    eval("global.testers.effectTesterFx = new Effect(" + this.effLength + ", e => {try{" + this.message + "}catch(er){Draw.color(Pal.remove); Lines.stroke(e.fout() * 5); Lines.circle(e.x, e.y, e.fin(Interp.pow10Out) * 30); Fonts.def.draw(err.toString(), e.x, e.y + e.fin(Interp.pow3Out) * 15, Pal.remove, 0.4, false, 1)}})")
